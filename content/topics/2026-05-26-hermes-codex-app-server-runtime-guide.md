...
### runtime
같은 일을 하더라도 <strong>어느 작업대에서 처리하느냐</strong>에 가깝습니다.
집에서 메모를 쓰는 것과, 공방에 가서 연장을 꺼내 본격적으로 작업하는 것의 차이라고 보면 됩니다.

### app-server
뒤에서 실제 작업을 받아 처리하는 <strong>전용 작업실</strong> 같은 느낌입니다.
겉에서는 같은 사람과 이야기하고 있어도, 실제 손일은 안쪽 작업실에서 돌아가는 구조라고 보면 됩니다.

### shell
컴퓨터에게 직접 일을 시키는 <strong>명령 창구</strong>입니다.
사람으로 치면 "이 파일 열어", "이 폴더 찾아", "이 작업 실행해" 하고 바로 지시하는 창구에 가깝습니다.

### apply_patch
파일을 통째로 다시 쓰기보다, <strong>어느 줄을 어떻게 고칠지 표시해서 바꾸는 수정 지시서</strong>에 가깝습니다.
종이 문서에 빨간 펜으로 "여기는 지우고, 여기는 이렇게 바꾸자" 표시하는 느낌으로 보면 됩니다.

### plugin
원래 있던 도구함에 <strong>새 기능을 꽂아 넣는 부속</strong>입니다.
드라이버 본체에 다른 비트를 끼워서 용도를 넓히는 것과 비슷합니다.

### MCP
AI가 바깥 도구와 연결될 때 쓰는 <strong>표준 연결 어댑터</strong> 같은 것입니다.
멀티탭이나 변환 젠더처럼, 서로 다른 도구를 한 통로로 붙여주는 감각으로 이해하면 됩니다.

### callback
한쪽에서 일을 하다가, 필요한 순간에 <strong>다시 원래 쪽으로 물어보는 되돌림 호출</strong>입니다.
식당 주방에서 손님 요청을 다 못 처리하면 홀로 다시 확인을 보내는 것과 조금 비슷합니다.

### sandbox
컴퓨터 안에서 작업해도 되는 <strong>울타리 친 연습장</strong>입니다.
아이들이 운동장 안에서 뛰게 하듯, 정해진 범위 안에서만 안전하게 움직이게 하는 장치라고 보면 됩니다.

### default runtime
특별히 바꾸지 않았을 때 쓰는 <strong>기본 작업 모드</strong>입니다.
자동차로 치면 시동 걸면 가장 먼저 들어가는 기본 주행 모드 같은 느낌입니다.

### codex_app_server
OpenAI/Codex 계열 작업을 할 때, Hermes가 자기 방식 대신 <strong>Codex 작업실에 넘겨서 처리하게 하는 모드 이름</strong>입니다.
쉽게 말하면 "이번 작업은 내가 직접 하지 말고, 저 전문 작업대에서 해"라고 고르는 스위치에 가깝습니다.
