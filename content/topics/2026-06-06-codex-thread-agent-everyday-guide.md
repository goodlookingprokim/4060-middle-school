...
이번 글은 기존 정리를 바탕으로, 시민개발자 구씨가 소개한 Codex의 새 thread 관리 설명도 함께 반영해 조금 더 실전적으로 다듬었습니다. 글 끝에 출처를 따로 적어두겠습니다.
...
그런데 최근에는 여기서 한 단계 더 나아가, "아예 방을 나눠서 일시키는 감각"으로 이해하면 더 편해졌습니다.

- 대표 방 하나는 전체 방향을 잡고
- 작업 방 여러 개는 각자 다른 일을 맡고
- 중간 결과는 다시 대표 방으로 가져오는 식입니다

예를 들면 이런 식입니다.

- 대표 방: 오늘 이 기능을 어디까지 만들지 결정
- 작업 방 1: 에러 원인 찾기
- 작업 방 2: 문서 초안 쓰기
- 작업 방 3: 테스트만 따로 돌려보기

이렇게 해두면 한 방에서 "코드 고치기, 글 쓰기, 검토하기, 정리하기"를 다 섞어 말하지 않아도 됩니다.
...
여기에 하나만 더 붙이면 판단이 더 쉬워집니다.

- 전체를 조율할 기준 방이 필요하면 대표 스레드
- 각 방이 서로 다른 성격의 일을 하면 병렬 스레드
- 한 방 안에서 잠깐 조사나 정리만 맡기면 서브에이전트
...
조금 더 현실적인 예시로 바꾸면 이렇습니다.

- 코드 수정은 한 스레드
- 글 설명 정리는 다른 스레드
- 중간 결과를 합쳐 최종 판단하는 곳은 대표 스레드

이 구조가 좋은 이유는 단순합니다. 일이 섞이지 않고, 맡긴 흔적이 남고, 누가 무슨 일을 했는지 다시 보기가 쉽습니다.

특히 아래처럼 작업 종류가 다른 경우에 스레드 분리가 잘 맞습니다.

- 구현과 리뷰를 나눌 때
- 디버깅과 문서 정리를 동시에 할 때
- 초안 작성과 사실 확인을 따로 굴릴 때
- 중간 산출물을 받아 다른 사람이 이어받아야 할 때

쉽게 말해, 일이 복잡해서가 아니라 일의 결이 다를 때 나누는 겁니다.

## 대표 방과 작업 방으로 생각하면 더 덜 헷갈립니다

시민개발자 구씨 설명에서 인상적인 부분도 이 감각이었습니다. 대표 방 하나가 있고, 옆에 작업 방이 여러 개 붙어 있는 구조로 보면 훨씬 쉽습니다.

대표 방은 이런 일을 맡습니다.

- 오늘 목표를 정함
- 어떤 일을 어느 방으로 보낼지 결정함
- 중간 보고를 받아 최종 방향을 다시 잡음
...
중요한 건, 작업 방이 오래 떠들수록 좋은 게 아니라는 점입니다. 맡은 일을 분명히 끝내고, 필요한 중간 결과만 들고 대표 방으로 돌아오는 게 더 좋습니다.
...
요즘 말로 하면 이 대표 스레드가 거의 팀장 방입니다. 작업 방이 아무리 많아도, 최종 판단과 우선순위는 여기서 잡는다고 생각하면 됩니다.
...
| 작은 일까지 전부 새 스레드로 나눔 | 오히려 컨텍스트만 흩어지고 토큰도 더 듦 | 정말 갈래가 다른 일만 분리함 |
| 불안해서 같은 설명을 여러 방에 길게 반복함 | 비슷한 맥락을 계속 다시 읽게 되어 낭비가 커짐 | 핵심 지시만 짧게 주고 중간 요약을 재사용함 |
...
여기에 하나 더 보태면, 스레드를 나누는 건 만능이 아닙니다.

- 아직 방향도 안 잡혔는데 방부터 많이 만들면 더 어수선해집니다.
- 아주 짧은 일을 괜히 새 스레드로 빼면 오히려 토큰과 시간이 더 듭니다.
- 실험적인 방이 많아질수록 결과 품질이 들쑥날쑥할 수도 있습니다.

그래서 처음에는 한 방에서 시작하고, 정말로 일이 갈라질 때만 나누는 편이 대체로 안정적입니다.
...
- 이 일을 나누면 정말 빨라지는가, 아니면 그냥 복잡해지기만 하는가
- 새 스레드가 필요한가, 아니면 중간 위임만으로 충분한가
...
그리고 새 기능이 생겼다고 해서 무조건 스레드를 많이 쪼갤 필요는 없습니다. 서로 다른 종류의 일, 병렬로 돌리면 빨라지는 일, 중간 결과를 건네받아 이어갈 일이 있을 때만 나누면 됩니다. 그 기준만 잡혀도 훨씬 덜 헷갈립니다.

## 출처

- 시민개발자 구씨, Codex의 새 thread 관리 기능 설명 요약, LilysAI 정리 링크: <https://lilys.ai/digest/9960114/11566756?s=1&noteVersionId=8107523>
