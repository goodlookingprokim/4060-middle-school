...
코난쌤(한준구)이 쓴 《이게 되네? 오픈클로 미친 활용법 50제》를 보다가, Quartz 블로그를 다룰 때도 초보자에게는 “한 번에 다 해줘”보다 **단계를 나눠서 시키는 프롬프트**가 훨씬 잘 맞겠다는 생각이 들었습니다.

블로그를 처음 만들 때 많은 분들이 설치 명령어부터 찾습니다. 물론 설치는 필요합니다. 그런데 실제로는 설치 하나로 끝나지 않습니다. **설치 → 확인 → 연결 → 작성 → 배포 → 검수** 흐름이 잡혀 있어야 중간에 덜 꼬입니다.

예를 들면 이런 식입니다.

1. **Quartz 설치 요청 프롬프트**
   - macOS 기준으로 Git, Node.js 설치 여부부터 확인하게 하고
   - Quartz 설치 명령어를 순서대로 설명하게 하고
   - 설치가 끝난 뒤 어느 폴더에 무엇이 생겨야 정상인지까지 보게 합니다.

2. **설치 확인 프롬프트**
   - Git, Node.js, npm이 잘 잡혀 있는지
   - Quartz 폴더가 정상인지
   - `npx quartz build`가 실제로 돌아가는지
   차례대로 확인하게 합니다.

3. **GitHub 저장소와 GitHub Pages 연결 프롬프트**
   - GitHub 저장소 만들기
   - 로컬 Quartz 폴더와 저장소 연결하기
   - GitHub Pages로 공개되게 설정하기
   - 최종 주소가 실제로 열리는지 확인하기
   여기까지 이어지게 합니다.

4. **글 초안 생성 프롬프트**
   - 제목, 도입, 소제목, 마무리 구조를 갖춘 글을 쓰게 하고
   - frontmatter까지 함께 만들게 해서
   - Quartz에 바로 넣을 수 있게 준비합니다.

5. **포스팅 업로드 프롬프트**
   - 마크다운 파일을 어디에 저장해야 하는지
   - 왜 `npx quartz build`를 하는지
   - 왜 `git add`, `git commit`, `git push`가 필요한지
   순서대로 이해하게 합니다.

6. **최종 주소 확인 프롬프트**
   - 블로그 홈이 열리는지
   - 새 글 주소가 정상인지
   - 마크다운 문법이 그대로 보이지 않는지
   - 모바일에서도 읽기 괜찮은지
   마지막까지 확인하게 합니다.

결국 좋은 프롬프트는 “블로그 만들어줘”가 아닙니다. **지금 내 단계에 맞게, 다음 한 걸음을 시키는 프롬프트**에 더 가깝습니다.

초보자일수록 이 순서가 도움이 됩니다. 한꺼번에 다 하려 하면 금방 꼬이기 쉽기 때문입니다. 그래서 Quartz도 단순한 블로그 툴로 보기보다, **AI와 함께 정리하고 발행하는 흐름**으로 보면 훨씬 실용적입니다.
