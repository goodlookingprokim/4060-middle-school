...
## 1. Quartz 블로그는 설치보다 흐름이 더 중요합니다

Quartz 블로그를 처음 만들 때는 보통 설치 명령어부터 찾게 됩니다. 물론 설치는 필요합니다. 그런데 실제로는 설치보다 **설치 → 확인 → 연결 → 작성 → 배포 → 검수** 흐름을 잡는 일이 더 중요합니다.

좋은 프롬프트도 비슷합니다. “블로그 만들어줘”처럼 한 번에 밀어붙이기보다, 지금 내가 어느 단계에 있는지 먼저 분명히 하고 거기에 맞는 질문을 던지는 편이 훨씬 낫습니다. 아직 설치 전이라면 Git과 Node부터 확인하고, 설치가 끝났다면 빌드와 GitHub Pages 연결을 차례로 점검하는 식입니다.

초보자일수록 이 순서가 도움이 됩니다. 한꺼번에 다 하려 하면 금방 꼬이기 쉽기 때문입니다. 오늘은 Quartz를 단순한 블로그 툴이 아니라, **AI와 함께 정리하고 발행하는 흐름**으로 보면 좋겠습니다.
