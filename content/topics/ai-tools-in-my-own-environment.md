...
아래는 입문자 교육을 위해 공유해주신 원문입니다.

```text
CLAUDE.md 이것은 Claude Code에 대한 작업 규칙 문서로, AI의 폭주를 방지하기 위해 필요합니다. Claude Code는 자율적으로 작동하기 때문에 규칙이 없으면 선의로 임의로 사양을 확장하거나 불필요한 기능을 만들어버릴 수 있습니다. 만드는 방법은 아래의 프롬프트를 실행하는 것입니다 ————————————————————— ~의 시스템을 만들고 싶어. 이 프로젝트용 CLAUDE.md를 만들어 주세요. 내용은 작업 규칙, 기술 스택, 금지 사항, 참조해야 할 설계서를 포함해 주세요. CLAUDE.md는 짧게, 200줄 이내에 요약해 주세요. —————————————————————
SPEC.md 이것은 시스템의 명세서이며, "무엇을 만들 것인가"를 고정하기 위한 것입니다. 이것이 없는 상태로 시스템을 만들어도, 나중에 다양한 기능이 부족해져서 사용할 수 없게 됩니다. 만드는 방법은 아래의 프롬프트를 실행하는 것입니다 ————————————————————— ~을 만들고 싶어. PC의 웹 브라우저에서 작동하며, 태스크의 추가, 완료, 삭제가 가능하면 좋겠습니다. 이 내용을 바탕으로, SPEC.md를 만들어 주세요. 부족한 명세가 있다면, 먼저 질문을 해 주세요. —————————————————————
TASKS.md
이것은 구현 절차서·체크리스트이며, 한 번에 만들게 하지 않고 순서대로 만들게 하기 위한 것입니다.
한 번에 전부 만들게 하면, 버그가 발생했을 때 원인이 파악하기 어려워집니다. TASKS.md가 있으면, 지금 어디까지 끝났는지, 다음에 무엇을 할지,가 명확해집니다.
만드는 방법은 아래의 프롬프트를 실행하는 것입니다. ————————————————————— SPEC.md의 내용을 바탕으로, 구현 태스크를 작게 분해해서 TASKS.md를 만들어 주세요. 각 태스크는 체크박스 형식으로 해 주세요. 구현이 끝날 때마다 TASKS.md를 업데이트하는 운영으로 해 주세요. —————————————————————
마지막으로, 「이 3점을 사용해서 구현 계획을 만들어」라고 Plan 모드에서 계획을 세워달라고 하고, TASK.md의 순서대로 구현해」라고 부탁하세요.
이 3점을 만들 때도, 「AskUserQuestion으로 진행해」라고 지시하면, Claude Code가 자신에게 확인을 받으면서 진행해 주기 때문에, 더 이상적인 것이 만들어지게 됩니다.
—————————————————————
위 내용으로 입문자 교육을 진행할거야. 단계적으로 진행할 수 있는 예시 프롬프트 3개 생성해줘. todo 웹앱 제작 기준으로.
```
