...
#### 그렇다고 모든 대화에 다 답하게 두는 뜻은 아닙니다

여기서 초보자가 가장 많이 헷갈리는 부분이 있습니다.

```text
require_mention: false로 바꿨다
=
이제 Hermes가 채널의 모든 말에 계속 끼어든다
```

이렇게 이해하면 곤란합니다.
실제 운영은 보통 <strong>수신은 열어두고, 발화는 선별하는 방식</strong>으로 가는 편이 낫습니다.

현재 운영 감각을 쉽게 풀어 쓰면 이렇습니다.

- `require_mention: false` = 멘션이 없어도 같은 채널 메시지를 Hermes가 읽을 수 있다.
- 그룹채팅 운영 지침 = 직접 질문, 도움 요청, 중요한 정정, 정리할 가치가 있는 말에는 답한다.
- 단순 잡담, 자연스럽게 흐르는 대화, 굳이 끼어들 필요 없는 말에는 조용히 있는다.
- 실제로 답할 필요가 없다고 판단하면 내부적으로 `NO_REPLY`처럼 무응답 처리해 채널에는 아무 말도 내보내지 않는다.

즉, 핵심은 이겁니다.

```text
수신을 연다 = 모든 메시지를 읽을 수 있게 한다
항상 발화한다 = 아니다
필요한 경우에만 답한다 = 맞다
```
...
- 멘션을 안 붙여도 필요한 요청은 잡을 수 있다.
- 그렇다고 봇이 모든 잡담에 끼어들어 채널을 시끄럽게 만들지는 않는다.
- 운영방에서는 사람 흐름을 깨지 않으면서 필요한 순간에만 보조하는 역할을 하게 된다.

그래서 `require_mention: false`는 단독으로 이해하면 안 되고, <strong>그 위에 어떤 운영 지침을 얹어 두느냐</strong>까지 같이 봐야 합니다.
...
## 18. 추가 학습 자료 — Hermes와 Discord를 조금 더 깊게 보고 싶다면

여기까지 따라왔다면 이제 이런 궁금증이 생길 수 있습니다.

- Hermes를 그냥 혼자 답하는 봇이 아니라, 여러 역할로 나눠 팀처럼 굴릴 수 있을까
- Discord 안에서 여러 에이전트가 서로 방해하지 않고 움직이게 하려면 무엇을 봐야 할까
- VPS에 올려서 24시간 켜두는 개인 비서 흐름은 어디까지 가능한가

이 질문에 도움 되는 자료를 출처별로 나눠서 정리해 둡니다.

### 먼저 출처를 정확히 나누면

이번 섹션은 아래 네 갈래를 바탕으로 정리했습니다.

#### 1) 공식 GitHub 저장소
- Hermes Agent 공식 GitHub 저장소  
...
#### 3) 제공받은 요약 링크
- Hermes Agent&Discord 활용한 멀티 에이전트 팀 셋업 가이드 요약 링크  
  <https://lilys.ai/digest/9728405/11259618?s=1&noteVersionId=7789055>
- 현시점 최고의 24시간 AI 비서? Hermes Agent 꼭 써보세요! 요약 링크  
  <https://lilys.ai/digest/9728062/11259154?s=1&noteVersionId=7788586>

중요한 점이 하나 있습니다.

위 두 Lilys 링크는 <strong>원문이나 영상 자체가 아니라 요약 링크</strong>입니다. 그래서 아래 정리에서는 Lilys 링크에 적힌 방향은 참고하되, 실제 동작 구조나 설정 원리는 가능한 한 <strong>공식 GitHub 저장소와 공식 문서에서 직접 확인되는 내용</strong>을 중심으로 설명합니다.

### 1. Hermes는 왜 멀티 에이전트 팀 운영에 어울리나

공식 GitHub 저장소를 보면 Hermes는 단순 채팅 봇보다 훨씬 넓게 설계되어 있습니다.

특히 눈에 띄는 포인트는 이런 것들입니다.

- Discord, Telegram, Slack 같은 여러 채널을 한 게이트웨이로 묶어 다룰 수 있음
- 서브에이전트를 분리해 병렬로 돌릴 수 있음
- 스케줄 작업을 자연어 기반으로 등록할 수 있음
- 메모리, 세션 검색, 과거 대화 회수 같은 흐름이 기본 구조 안에 들어 있음

쉽게 말하면,
Hermes는 "한 번 대답하고 끝나는 챗봇"보다 <strong>계속 일하고, 다시 불러 쓰고, 역할을 나눠 굴릴 수 있는 작업형 에이전트</strong> 쪽에 더 가깝습니다.

### 2. Discord에서 진짜 중요한 건 봇 수보다 규칙이다

요약 자료에서는 멀티 에이전트 팀, 역할 분담, 팀 구조 같은 표현이 강조됩니다. 실제로 그런 운영 그림은 충분히 가능합니다. 다만 공식 Discord 문서를 직접 보면, 기술적으로 먼저 중요한 건 화려한 조직도가 아니라 <strong>응답 규칙</strong>입니다.

공식 Discord 문서에서 특히 중요한 부분은 아래입니다.

- 기본적으로 서버 채널에서는 <strong>멘션될 때만</strong> 답한다
- 특정 채널만 예외적으로 자유 응답 채널로 열 수 있다
- 여러 사람이 같은 채널에서 말해도 기본값에서는 사용자별 세션이 분리된다
- 다른 사람을 멘션한 메시지에는 봇이 끼어들지 않게 막는 옵션이 있다

이 말은 곧,
멀티 에이전트 팀을 만들 때도 먼저 "누가 어느 방에서, 어떤 조건에서, 누구에게만 반응하나"가 정리돼야 한다는 뜻입니다.

봇이 셋이든 일곱이든, 이 규칙이 흐리면 협업이 아니라 소음이 됩니다.

### 3. 역할 분담은 거창하게 시작할 필요가 없다

요약 자료에서는 7개 역할 계층 구조, TFT(Task Force Team) 프로토콜 같은 운영 감각이 강조돼 있습니다. 이런 방식은 실제로 팀 운영 아이디어를 떠올리는 데는 도움이 됩니다.

다만 4060미들스쿨 기준에서는 처음부터 조직도를 크게 잡기보다, 아래처럼 시작하는 편이 훨씬 현실적입니다.
...
즉,
처음에는 <strong>큰 체계보다 역할이 겹치지 않는 작은 팀</strong>이 더 중요합니다.

이 관점에서 보면 우리가 이 글에서 다룬 구조도 이미 하나의 팀 운영 예시입니다.

- OpenClaw = 팀장
- Hermes = 팀원
- Discord = 공용 작업 공간

여기서 더 늘리고 싶으면, 그다음에만 역할을 추가하면 됩니다.

### 4. SQLite WAL 이야기가 왜 실무적으로 중요할까

공식 세션 저장소 문서에서 확인되는 중요한 포인트 중 하나가 <strong>SQLite WAL 모드</strong>입니다.

Hermes는 `~/.hermes/state.db`를 사용해 세션 메타데이터, 전체 메시지 기록, 모델 설정, 검색용 인덱스를 저장합니다. 그리고 기본 설계에서 WAL 모드를 사용합니다.
...
에서 읽기와 쓰기가 조금 더 안전하게 겹쳐질 수 있기 때문입니다.

쉽게 말하면,
<strong>여러 사람이 같은 공책을 동시에 보더라도, 한 사람이 쓰는 동안 전체가 완전히 멈추지 않게 만드는 방식</strong>에 가깝습니다.

그래서 요약 자료에 나온 "SQLite WAL 경쟁 패치" 같은 표현은, 방향 자체는 공식 문서의 WAL 설계와 맞닿아 있다고 볼 수 있습니다. 다만 세부 표현은 요약 자료의 언어일 수 있으니, 실제 원리는 공식 `session-storage.md`를 함께 보는 편이 좋습니다.

### 5. 24시간 AI 비서라는 말은 어디까지 현실적인가

두 번째 요약 자료에서는 Hermes를 24시간 작동하는 개인 비서처럼 설명합니다. 이건 과장이기만 한 표현은 아닙니다.

공식 저장소를 보면 Hermes는 원래부터 아래 같은 흐름에 강합니다.

- gateway 기반 메시징 연결
- cron 스케줄 자동화
- 여러 플랫폼에서 같은 에이전트 이어 쓰기
- VPS 같은 곳에 올려 두고 계속 접근하기

즉,
노트북 안에서 잠깐 켜는 실험을 넘어
<strong>내가 자주 쓰는 채널에서 계속 불러 쓸 수 있는 비서형 구조</strong>로 보는 게 맞습니다.

다만 여기서도 환상을 조금 걷어내는 게 좋습니다.

24시간 AI 비서라는 말은
"완벽하게 다 알아서 하는 존재"라기보다,
<strong>반복 업무를 계속 받아주고, 내가 자주 시키는 흐름을 기억하는 작업 동반자</strong>에 가깝습니다.

이렇게 보면 기대치가 훨씬 건강해집니다.

### 6. 이 자료들을 어떻게 읽으면 좋을까

처음 보는 분이라면 아래 순서가 덜 헷갈립니다.

#### 1단계
먼저 이 글과 1편으로 전체 흐름을 잡습니다.
- Android/Termux 설치
- Discord 연결
- OpenClaw와 팀장-팀원 구조

#### 2단계
그다음 공식 Discord 문서를 봅니다.
여기서 특히 아래만 먼저 챙기면 됩니다.
- mention 규칙
- free-response 채널
- 사용자별 세션 분리
...
#### 3단계
그다음 공식 GitHub 저장소를 훑어봅니다.
이 단계에서는 전체 철학과 지원 범위를 보는 느낌이면 충분합니다.
- gateway
- delegation
- scheduling
- profiles
...
#### 4단계
마지막으로 Lilys 요약 링크를 봅니다.
이건 설치 매뉴얼보다 <strong>"실제로 이렇게 굴릴 수도 있겠구나" 하는 운영 상상력</strong>을 넓히는 자료로 보는 편이 좋습니다.

### 7. 4060미들스쿨 기준으로 다시 한 번 줄이면

이 자료들을 다 보고 나서 남았으면 하는 감각은 이것입니다.

- Hermes는 메신저에 답하는 봇 하나로 끝나는 도구가 아니다
- Discord 안에서 역할을 나눠 팀처럼 운영할 수 있다
- 중요한 건 봇 개수보다 응답 규칙과 역할 경계다
- 장기 운영에서는 세션 저장 구조와 자동화 방식이 꽤 중요하다
- VPS, 스케줄, 메신저 연결을 묶으면 개인 비서형 흐름으로 갈 수 있다

즉,
Hermes를 진짜 잘 쓰는 방향은 "어떤 모델이 제일 똑똑하냐"보다,
<strong>어떤 역할을 맡기고, 어떤 채널에서, 어떤 규칙으로 계속 굴리느냐</strong>에 더 가깝습니다.
